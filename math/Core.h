...
namespace math
{
	constexpr static float PI = 3.14159265359f, E = 2.71828182246f;
	...
	static const A& clamp(const A& a, const B& b, const C& c)
	{
	...
	static A dist(const A& x1, const B& x2, const C& y1, const D& y2)
	{
	...
	static bool equalsIgnoreCase(const std::string& a, std::string& b)
	{
		if (a.size() != b.size())
			return false;
